...
class AlbumListScene(PiScene):
    ...
    def make_scroll_view(self):
        scroll_list = ui.ScrollList(
            ui.Rect(
                ...
                self.main.frame.height - self.btn_size - self.margins * 2 - 10
            ),
            ui.Rect(
                0,0,
    ...
    def make_page_nav(self):
        icon_down = ui.IconButton(ui.Rect(0,0,self.btn_size,self.btn_size),'chevron-down',12)
        btn_down = ui.Button(ui.Rect(icon_down.frame.right,0,60,self.btn_size),'Page',halign=ui.CENTER,valign=ui.CENTER)
        
        icon_up = ui.IconButton(ui.Rect(btn_down.frame.right+self.margins,0,self.btn_size,self.btn_size),'chevron-up',12)
        btn_up = ui.Button(ui.Rect(icon_up.frame.right,0,60,self.btn_size),'Page',halign=ui.CENTER,valign=ui.CENTER)
    ...
    def populate_artists_view(self):
        ...
        for artist in artists:
            ...
            add_all_btn = ui.IconButton( ui.Rect(artist_button.frame.right + self.margins,scr_y,self.btn_size,self.btn_size), 'plus', 12 )
